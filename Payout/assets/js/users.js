...
addForm.addEventListener("submit", async (e) => {
  ...
  if (addForm.checkValidity() === false) {
    ...
    const data = await fetch("users.controller.php", {
      method: "POST",
      body: formData,
    });
...
const fetchAllUsers = async () => {
  const data = await fetch("users.controller.php?read=1", {
    method: "GET",
  });
  const response = await data.text();
...
const editUser = async (id) => {
  const data = await fetch(`users.controller.php?edit=1&id=${id}`, {
    method: "GET",
  });
  ...
  document.getElementById("profile").value = response.profile;
...
const deleteUser = async (id) => {
  const data = await fetch(`users.controller.php?delete=1&id=${id}`, {
    method: "GET",
  });
  const response = await data.text();
  showAlert.innerHTML = response;
